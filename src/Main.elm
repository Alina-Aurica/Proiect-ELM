...
getRepo = Http.get
    {
        url = "https://api.github.com/users/octocat/repos", --url-ul pt a extrage Repo-ul
...
update msg model =
    case msg of
        ...
        SelectEventCategory category -> 
            ( {model | selectedEventCategories = Model.Event.Category.set category True model.selectedEventCategories}, Cmd.none )

        DeselectEventCategory category ->
            ( {model | selectedEventCategories = Model.Event.Category.set category False model.selectedEventCategories}, Cmd.none )
