...
-- calculez lungimea intervalului in luni cu afisare diferita
lungimeInterval : Interval -> Maybe Int
lungimeInterval (Interval interval) =
    Maybe.andThen (Date.monthsBetween interval.start) <| interval.end
...
compare (Interval intA) (Interval intB) =
    chainCompare (Basics.compare (lungimeInterval (Interval intA) |> Maybe.withDefault 99) (lungimeInterval (Interval intB) |> Maybe.withDefault 99)) (Date.compare intA.start intB.start)
...
view (Interval interval) =
    let
       ...
       lenInterval (Interval interv) = 
            case (length (Interval interv)) of
                Just (y, m) -> text ((String.fromInt y) ++ " " ++ (String.fromInt m))
                Nothing -> text "" 
    in
    div [class "interval"] [
        ...
        div [class "interval-end"] [
            if interval.end == Nothing then
               ...
               (Date.view <| Maybe.withDefault (Date.onlyYear 2000) <| interval.end)
        ],
        div [class "interval-length"] [lenInterval (Interval interval)]
    ]
